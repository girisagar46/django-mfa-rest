...
def security_settings(request):
    ...
    return render(
        request,
        "django_mfa_rest/security.html",
        {
            "prev_url": settings.LOGIN_REDIRECT_URL,
...
def configure_mfa(request):
    ...
    return render(
        request,
        "django_mfa_rest/configure.html",
...
def disable_mfa(request):
    ...
    return render(request, "django_mfa_rest/disable_mfa.html")
...
def verify_second_factor_totp(request):
    ...
    if request.method == "GET":
        ...
        return render(request, "django_mfa_rest/verify_second_factor_mfa.html", ctx)
    ...
    return render(
        request, "django_mfa_rest/verify_second_factor_mfa.html", ctx, status=400
    )
    ...
    if request.method == "GET":
        if is_mfa_enabled(request.user):
            if UserRecoveryCodes.objects.filter(
                ...
                codes = UserRecoveryCodes.objects.values_list(
                    "secret_code", flat=True
                ).filter(user=UserOTP.objects.get(user=request.user.id))
            else:
            ...
            return render(
                request,
                "django_mfa_rest/recovery_codes.html",
                {"codes": codes, "next_url": next_url},
            )
        else:
...
def verify_second_factor(request):
    if request.method == "GET":
        ...
        if twofactor_enabled and u2f_enabled:
            return render(
                request,
                "django_mfa_rest/verify_second_factor.html",
                {"u2f_enabled": u2f_enabled, "twofactor_enabled": twofactor_enabled},
            )
        if u2f_enabled:
...
class AddKeyView(OriginMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        request = u2f.begin_registration(
            self.get_origin(),
