...
from django_mfa_rest import totp
...
class Views_test_auth_factor(TestCase):
    def setUp(self):
        ...
        UserOTP.objects.create(otp_type="TOTP", user=self.user, secret_key="secret_key")
    ...
    def test_security_settings_view(self):
        response = self.client.get(reverse("mfa:security_settings"))
        self.assertTemplateUsed(response, "django_mfa_rest/security.html")

    def test_configure_mfa_view(self):
        response = self.client.get(reverse("mfa:configure_mfa"))
        self.assertTemplateUsed(response, "django_mfa_rest/configure.html")

    def test_verify_second_factor_totp_using_recovery_codes(self):
        response = self.client.post(
            reverse("mfa:verify_second_factor_totp"), {"verification_code": "ABcDg"}
        )
    ...
    def test_verify_second_factor_u2f_view(self):
        ...
        session["verfied_u2f"] = False
        session.save()
        response = self.client.get(reverse("mfa:verify_second_factor_u2f"), follow=True)
        url = reverse("mfa:verify_second_factor") + "?next=" + settings.LOGIN_URL
        self.assertRedirects(response, expected_url=url, status_code=302)

        session["u2f_pre_verify_user_pk"] = auth.get_user(self.client).id
        session[
            "u2f_pre_verify_user_backend"
    ...
    def test_recovery_codes_already_generated_view(self):
        ...
        self.assertTemplateUsed(response, "django_mfa_rest/recovery_codes.html")
    ...
    def test_verify_second_factor_view(self):
        ...
        self.assertTemplateUsed(response, "django_mfa_rest/verify_second_factor.html")
    ...
    def test_key_list_view(self):
        ...
        response = self.client.post(
            reverse("mfa:u2f_keys"), {"key_id": self.u2f.id, "delete": True}
        )
        self.assertEquals(response.status_code, 302)
    ...
    def test_disable_mfa_view(self):
        ...
        self.assertTemplateUsed(response, "django_mfa_rest/disable_mfa.html")
...
class Views_test_not_auth_factor(TestCase):
    ...
    def test_recovery_codes_view(self):
        response = self.client.get(reverse("mfa:recovery_codes"))
        self.assertEquals(response.content, b"please enable twofactor_authentication!")
...
class Views_test_auth_factor_new(TestCase):
    ...
    def test_key_list_view(self):
        ...
        self.assertTemplateUsed(response, "u2f/key_list.html")
        response = self.client.post(
            reverse("mfa:u2f_keys"), {"key_id": self.u2f1.id, "delete": True}
        )
        self.assertEquals(response.status_code, 302)
        self.assertEquals(response.url, reverse("mfa:u2f_keys"))
