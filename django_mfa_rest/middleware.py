...
class MfaMiddleware(MiddlewareMixin):
    def process_request(self, request):
        if request.user.is_authenticated and (
            (is_mfa_enabled(request.user) and not verify_rmb_cookie(request))
            or is_u2f_enabled(request.user)
        ):
            if not request.session.get("verfied_otp") and not request.session.get(
                "verfied_u2f"
            ):
                current_path = request.path
                paths = [
                    reverse("mfa:verify_second_factor"),
                    reverse("mfa:verify_second_factor_u2f"),
                    reverse("mfa:verify_second_factor_totp"),
                ]
